...
extern bool acc_ready, q_ready, gps_ready;
	...
	while(1){
		...
		if(!q_ready){
			...
			if(gps_ready){
				...
				if(gps_valid){
					double lat = atof(Gps_GetInfo->latitude);
					double lon = atof(Gps_GetInfo->longitude);
					lat = gps_dm_2_dd(lat);
					lon = gps_dm_2_dd(lon);
					double track_rad = Gps_GetInfo->Course * PI / 180.0f;
					float vel_n_m_s = Gps_GetInfo->Speed / 3.6f * cosf(track_rad);	// N
					float vel_e_m_s = Gps_GetInfo->Speed / 3.6f * sinf(track_rad);	// E
					...
					if(!ref_inited){
						...
						// get the refer location after 1 second
							...
							Trace(3, "init ref: lat=%.7f, lon=%.7f", (double)lat, (double)lon);
						}
					}
